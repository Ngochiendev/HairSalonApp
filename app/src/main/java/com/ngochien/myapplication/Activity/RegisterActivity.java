...
public class RegisterActivity extends AppCompatActivity {
    ImageButton btnBack;
    EditText edtPassword,edtUsername,edtConfirmPassword,edtPhone;
    Button btnRegister;
    ...
    private void Register() {
        ...
        String Password2=edtConfirmPassword.getText().toString().trim();
        ...
        if(TextUtils.isEmpty(Phone)){
            edtPhone.setError("Hãy điền số điện thoại");
            return;
        }
        ...
        firebaseAuth.createUserWithEmailAndPassword(Phonetrick,Password1).addOnCompleteListener(new OnCompleteListener<AuthResult>() {
            @Override
            public void onComplete(@NonNull Task<AuthResult> task) {
                if(task.isSuccessful()){
                    ...
                    Toast.makeText(getApplicationContext().getApplicationContext(),"Lỗi ! " + task.getException().toString() ,Toast.LENGTH_LONG).show();
                }
            }
        });
    }
